...
COPY server.py index.html benchmark_avatar.py test_benchmark_handshake.py test_motion_continuity.py /workspace/FasterLivePortrait/
COPY entrypoint.sh /workspace/entrypoint.sh
RUN chmod +x /workspace/entrypoint.sh
RUN cd /workspace/FasterLivePortrait \
    && python -m unittest -v test_motion_continuity.py
